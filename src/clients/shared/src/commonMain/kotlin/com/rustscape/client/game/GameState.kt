...
data class BankItem(
    ...
    val isEmpty: Boolean get() = itemId <= 0 || (amount <= 0 && !placeholder)

    companion object {
        val EMPTY = BankItem(-1, -1, 0, 0, false)
    }
}

/**
 * Bank settings from server
 */
@Serializable
data class BankSettings(
    var noteMode: Boolean = false,
    var placeholders: Boolean = false,
    var withdrawXAmount: Int = 10
)
...
data class Item(
    ...
    /** Alias for id to match common naming convention */
    val itemId: Int get() = id
...
class Inventory(val capacity: Int = 28) {
    private val _items = Array(capacity) { Item.EMPTY }

    /** Get all items as a list */
    val items: List<Item> get() = _items.toList()

    fun get(slot: Int): Item {
        return if (slot in 0 until capacity) _items[slot] else Item.EMPTY
    }
    ...
    fun toList(): List<Item> = _items.toList()

    /** Find the first slot containing an item with the given id */
    fun findSlot(itemId: Int): Int = _items.indexOfFirst { it.id == itemId && !it.isEmpty }

    val freeSlots: Int get() = _items.count { it.isEmpty }
    val usedSlots: Int get() = _items.count { !it.isEmpty }
}
...
class GameState {
    ...
    // Bank state
    var bankOpen: Boolean = false
    var bankCapacity: Int = 800
    val bankSettings: BankSettings = BankSettings()
    private val _bankItems: MutableList<BankItem> = mutableListOf()
    val bankItems: List<BankItem> get() = _bankItems.toList()
    private val _bankTabSizes: IntArray = IntArray(9) { 0 }
    val bankTabSizes: IntArray get() = _bankTabSizes.copyOf()
    ...
    fun openBank(capacity: Int) {
        bankOpen = true
        bankCapacity = capacity
        notifyListeners { it.onBankOpened(capacity) }
    }

    fun closeBank() {
        bankOpen = false
        notifyListeners { it.onBankClosed() }
    }

    fun updateBankSettings(noteMode: Boolean, placeholders: Boolean, withdrawX: Int) {
        bankSettings.noteMode = noteMode
        bankSettings.placeholders = placeholders
        bankSettings.withdrawXAmount = withdrawX
        notifyListeners { it.onBankSettingsChanged(bankSettings) }
    }

    fun updateBankTabSizes(tabSizes: IntArray) {
        for (i in tabSizes.indices.take(_bankTabSizes.size)) {
            _bankTabSizes[i] = tabSizes[i]
        }
        notifyListeners { it.onBankTabsChanged(_bankTabSizes) }
    }

    fun setBankItems(items: List<BankItem>) {
        _bankItems.clear()
        _bankItems.addAll(items)
        notifyListeners { it.onBankItemsChanged(_bankItems) }
    }

    fun updateBankSlot(tab: Int, slot: Int, itemId: Int, amount: Int, placeholder: Boolean) {
        // Find existing item at this tab/slot and update, or add new
        val index = _bankItems.indexOfFirst { it.tab == tab && it.slot == slot }
        val newItem = BankItem(slot, itemId, amount, tab, placeholder)

        if (itemId <= 0 && amount <= 0 && !placeholder) {
            // Remove item
            if (index >= 0) {
            ...
            // Update existing
            _bankItems[index] = newItem
        } else {
            // Add new
            _bankItems.add(newItem)
        }
        notifyListeners { it.onBankSlotUpdated(tab, slot, itemId, amount, placeholder) }
    }

    fun clearBank() {
        _bankItems.clear()
        for (i in _bankTabSizes.indices) {
            _bankTabSizes[i] = 0
        }
        notifyListeners { it.onBankItemsChanged(_bankItems) }
    }

    fun getBankItemsForTab(tab: Int): List<BankItem> {
        return _bankItems.filter { it.tab == tab }.sortedBy { it.slot }
    }
    ...
    fun reset() {
        ...
        bankOpen = false
        bankCapacity = 800
        bankSettings.noteMode = false
        bankSettings.placeholders = false
        bankSettings.withdrawXAmount = 10
        _bankItems.clear()
        for (i in _bankTabSizes.indices) {
            _bankTabSizes[i] = 0
        }
...
interface GameStateListener {
    ...
    // Bank callbacks
    fun onBankOpened(capacity: Int) {}
    fun onBankClosed() {}
    fun onBankSettingsChanged(settings: BankSettings) {}
    fun onBankTabsChanged(tabSizes: IntArray) {}
    fun onBankItemsChanged(items: List<BankItem>) {}
    fun onBankSlotUpdated(tab: Int, slot: Int, itemId: Int, amount: Int, placeholder: Boolean) {}
}

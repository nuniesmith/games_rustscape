...
object ServerOpcode {
    ...
    /** Bank interface open (capacity) */
    const val BANK_OPEN = 248

    /** Bank contents update (full or slot) */
    const val BANK_UPDATE = 249

    /** Bank settings (note mode, placeholders, withdraw-X) */
    const val BANK_SETTINGS = 250

    /** Bank tab sizes info */
    const val BANK_TAB_INFO = 251

    // ===== EQUIPMENT OPCODES =====

    /** Equipment contents update */
    const val EQUIPMENT_UPDATE = 94

    /** Equipment bonuses update */
    const val EQUIPMENT_BONUSES = 95
}
...
object ClientOpcode {
    ...
    /** Open bank interface */
    const val BANK_OPEN = 22

    /** Close bank interface */
    const val BANK_CLOSE = 185

    /** Withdraw item from bank */
    const val BANK_WITHDRAW = 43

    /** Deposit item to bank */
    const val BANK_DEPOSIT = 233

    /** Deposit all inventory to bank */
    const val BANK_DEPOSIT_ALL = 129

    /** Deposit worn equipment to bank */
    const val BANK_DEPOSIT_EQUIPMENT = 195

    /** Select bank tab */
    const val BANK_TAB_SELECT = 55

    /** Move item within bank (reorganize) */
    const val BANK_MOVE_ITEM = 214

    /** Search bank items */
    const val BANK_SEARCH = 116

    /** Toggle withdraw as note */
    const val BANK_NOTE_MODE = 117

    /** Set withdraw amount mode (1, 5, 10, X, All) */
    const val BANK_WITHDRAW_MODE = 115

    // ===== INVENTORY ACTION OPCODES =====

    /** Drop item from inventory */
    const val ITEM_DROP = 145

    /** Use item (generic) */
    const val ITEM_USE = 122

    /** Equip/wear item */
    const val ITEM_EQUIP = 41

    /** Swap inventory slots */
    const val INVENTORY_SWAP = 214

    /** Insert item at slot (shift mode) */
    const val INVENTORY_INSERT = 243
}
...
object PacketSize {
    ...
    val SERVER_SIZES = intArrayOf(
        ...
        1, 0, 0, 0, 0, 0, 0, 0, 2, -2, // 240-249: 248=BANK_OPEN(2), 249=BANK_UPDATE(var)
        6, -1, 0, -1, 6, 0             // 250-255: 250=BANK_SETTINGS(6), 251=BANK_TAB_INFO(var)
    )
